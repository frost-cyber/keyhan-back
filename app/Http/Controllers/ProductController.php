...
class ProductController extends Controller {
	...
	protected function save ( array $data , Product $product = NULL ) {
		...
		$product->is_virtual = $data[ 'is_virtual' ] ?? FALSE;
	...
	protected function syncVariables ( array $variablesData , Product $product ) {
		...
		foreach ( $variablesData as $index => $variableData ) {
			$variableValue = AttributeValue::find( $variableData[ 'id' ] );
			
			if ( $variableValue ) {
				$syncData[ $variableValue->id ] = [
				 'variable_id'      => $variableValue->attribute->id ,
		...
		$product->variables()->sync( $syncData );
	}
